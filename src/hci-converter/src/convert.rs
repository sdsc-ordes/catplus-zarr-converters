...
use catplus_common::models::types::Experiment;
...
pub fn json_to_rdf(input_content: &str, fmt: &str) -> Result<String> {
    ...
    let experiment: Experiment = parse_json(input_content).context("Failed to parse JSON input")?;

    // Build the RDF graph
    let mut graph_builder = GraphBuilder::new();
    graph_builder.insert(&experiment).context("Failed to build RDF graph")?;
...
fn parse_json(json_data: &str) -> Result<Experiment> {
